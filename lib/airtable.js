  ...
  try {
    ...
    return records.map(record => {
      ...
      return {
        ...
        salePrice: record.get('Sale Price') || null,
        category: record.get('Category') || 'Uncategorized',
        images: record.get('Images') || [],
        inStock: record.get('In Stock') !== false,
...
export async function validatePromoCode(code) {
  try {
    ...
    const record = records[0];
    const expiryDate = record.get('Expiry Date');
    ...
    return {
      ...
      discountType: record.get('Discount Type'), // 'percentage' or 'fixed'
      discountValue: record.get('Discount Value'),
      minPurchase: record.get('Min Purchase') || 0,
      maxDiscount: record.get('Max Discount'),
    };
  } catch (error) {
...
export async function getProductReviews(productId) {
  try {
    ...
    const records = await base(REVIEWS_TABLE)
      .select({
        filterByFormula: `AND({Product ID} = '${productId}', {Approved} = TRUE())`,
