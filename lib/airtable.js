...
export function getBase() {
  if (!baseInstance) {
    ...
    // The airtable client defaults to a 5-minute request timeout, which
    // means a stuck/unreachable request would leave pages hanging on
    // "loading" for minutes instead of failing fast into the existing
    // graceful-fallback (empty array/object) error handling below.
    baseInstance = new Airtable({ apiKey, requestTimeout: 15000 }).base(baseId);
  }
  return baseInstance;
}

// Simple in-memory cache for full-table reads. Pages already assume a
// `revalidate: 60` staleness window, so reusing data for the same window
// avoids re-fetching the same table on every request in dev (getStaticProps
// re-runs per-request outside of `next build`) and across the many pages
// that read the same data during a single build.
...
export async function getAllProducts() {
  ...
  try {
    ...
    return setCached('allProducts', products);
  } catch (error) {
...
export async function getProductReviews(productId) {
  try {
    ...
    // Filter by product server-side instead of scanning the whole reviews
    // table per product. ARRAYJOIN handles ProductId whether it's a linked
    // (array) field or a plain text field.
    const records = await base(REVIEWS_TABLE)
      .select({
        filterByFormula: `AND({Approved} = TRUE(), FIND('${productId}', ARRAYJOIN({ProductId})))`,
        sort: [{
          field: 'Created_At',
          direction: 'desc'
        }],
...
export async function getUserById(userId) {
  try {
    ...
    return {
      ...
      email: record.get('Email'),
      passwordHash: record.get('PasswordHash'),
      password: record.get('PasswordHash'), // Alias for compatibility
      firstName: record.get('FirstName'),
      lastName: record.get('LastName'),
      phone: record.get('Phone'),
      active: record.get('Active'),
...
export async function storeResetToken(userId, token, expiry) {
  try {
    ...
    // Store expiry as timestamp (number) to avoid Airtable select field issues
    const expiryTimestamp = new Date(expiry).getTime();

    await base(USERS_TABLE).update(userId, {
      ResetToken: token,
      ResetTokenExpiryTime: expiryTimestamp,
    });
...
export async function findUserByResetToken(token) {
  try {
    ...
    const record = records[0];
    const expiryTimestamp = record.get('ResetTokenExpiryTime');

    // Check if token is expired (compare timestamp)
...
export async function updateOrderPaymentStatus(orderNumber, paymentStatus, paymentData = {}) {
  try {
    ...
    if (paymentData.amountReceived) {
      updateData.AmountReceived = paymentData.amountReceived;
    }
    if (paymentData.error) {
      updateData.PaymentError = paymentData.error;
    }

    // Update status to Processing if payment succeeded
    if (paymentStatus === 'Paid') {
      updateData.Status = 'Processing';
    }

    await base(ORDERS_TABLE).update(record.id, updateData);
...
// Update order with multiple fields (renamed from the conflicting function in the original code)
...
const airtableAPI = {
  ...
  updateOrderStatus,
  updateOrderTracking,
  updateOrderPaymentStatus, // Corrected function name and logic
  updateOrder, // Kept the generic update function
  createAddress,
  getUserAddresses,
  updateAddress,
  deleteAddress,
