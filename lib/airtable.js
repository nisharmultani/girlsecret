...
/**
 * Transform Airtable product record to application format
 * Extracts duplication from getAllProducts, getProductById, getProductBySlug
 * @param {Object} record - Airtable record
 * @param {string} slug - Optional slug override
 * @returns {Object} Transformed product object
 */
function transformProductRecord(record, slug = null) {
  ...
  return {
    ...
    slug: slug || record.get('Slug') || name.toLowerCase().replace(/\s+/g, '-'),
...
export async function getAllProducts() {
  try {
    ...
    return records.map(record => transformProductRecord(record));
  } catch (error) {
    return [];
  }
}
...
export async function getProductBySlug(slug) {
  try {
    ...
    return transformProductRecord(record, slug);
  } catch (error) {
    return null;
  }
}
