...
export async function getProductReviews(productId) {
  try {
    ...
    const records = await base(REVIEWS_TABLE)
      .select({
        // For linked records, we need to check if the array contains the productId
        // ARRAYJOIN converts the linked record array to string for comparison
        filterByFormula: `AND(FIND('${productId}', ARRAYJOIN({ProductId})), {Approved} = TRUE())`,
