...
function mapProduct(row) {
  return {
    ...
    images: row.images || [],
    Available_Products: row.available_product_images || [],
...
function mapOrder(row) {
  return {
    ...
    supplierStatus: row.supplier_status || null,
...
// === ADMIN: PRODUCTS ===
// New in this migration: pages/api/admin/products/{create,update,delete}.js
// each instantiated their own Airtable client directly instead of going
// through lib/airtable.js.

function normalizeImageList(images) {
  return images.map(img => {
    if (typeof img === 'string') return { url: img };
    return { url: img.url || img.thumbnails?.large?.url };
  });
}
...
export async function createProduct(productData) {
  try {
    ...
    const row = {
      ...
      sold_count: productData.soldCount !== undefined ? parseInt(productData.soldCount) || 0 : 0,
      sizes: productData.sizes && productData.sizes.length > 0 ? productData.sizes : [],
      video_urls: productData.videoUrls && productData.videoUrls.length > 0 ? productData.videoUrls : [],
      images: productData.images && productData.images.length > 0 ? normalizeImageList(productData.images) : [],
      available_product_images: productData.availableProductImages && productData.availableProductImages.length > 0
        ? normalizeImageList(productData.availableProductImages) : [],
    };

    const { data, error } = await supabase.from('products').insert(row).select().single();
    if (error) throw error;

    return { success: true, product: { id: data.id, name: data.name } };
  } catch (error) {
...
export async function updateProduct(productId, updateData) {
  try {
    ...
    const row = {};
    if (updateData.name) row.name = updateData.name;
    if (updateData.description !== undefined) row.description = updateData.description;
    if (updateData.specifications !== undefined) row.specifications = updateData.specifications;
    if (updateData.price) row.price = parseFloat(updateData.price);
    if (updateData.category) row.category = updateData.category;
    if (updateData.slug) row.slug = updateData.slug;
    if (updateData.inStock !== undefined) row.in_stock = updateData.inStock;
    if (updateData.featured !== undefined) row.featured = updateData.featured;
    if (updateData.salePrice !== undefined) {
      row.sale_price = updateData.salePrice ? parseFloat(updateData.salePrice) : null;
    }
    if (updateData.sizes !== undefined) row.sizes = updateData.sizes;
    if (updateData.soldCount !== undefined) row.sold_count = parseInt(updateData.soldCount) || 0;
    if (updateData.videoUrls !== undefined) row.video_urls = updateData.videoUrls;
    if (updateData.images !== undefined) {
      row.images = updateData.images.length > 0 ? normalizeImageList(updateData.images) : [];
    }
    if (updateData.availableProductImages !== undefined) {
      row.available_product_images = updateData.availableProductImages.length > 0
        ? normalizeImageList(updateData.availableProductImages) : [];
    }
...
export async function deleteProduct(productId) {
  try {
    ...
    // Reviews and wishlist entries for this product are removed
    // automatically by the `on delete cascade` foreign keys in the schema
    // - no manual cleanup query needed here, unlike the old Airtable version.
...
// === ADMIN: ORDERS ===
// New in this migration: pages/api/admin/orders.js instantiated its own
// Airtable client directly. Returns the same PascalCase field shape the
// existing pages/admin/orders.js admin UI already reads.
...
export async function getAllOrders() {
  try {
    ...
    return data.map(row => ({
      ...
      Carrier: row.carrier,
      AliExpressStatus: row.supplier_status,
      Notes: row.notes,
      CreatedAt: row.created_at,
