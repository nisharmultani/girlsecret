...
export const addToCart = (product, quantity = 1, size = null, color = null, selectedImageUrl = null) => {
  const cart = getCart();

  // Use the selected image URL, or fall back to the first product image
  const imageUrl = selectedImageUrl || product.images[0]?.url || product.images[0]?.thumbnails?.large?.url || '';
  ...
  const existingItem = cart.find(item =>
    ...
    item.size === size &&
    item.color === color &&
    item.image === imageUrl // Also match by image to treat different variants separately
  );
