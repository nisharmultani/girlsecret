...
/**
 * Send welcome email with a discount promo code (used for the homepage welcome modal opt-in)
 * @param {string} email - Subscriber's email address
 * @param {string} promoCode - Discount code to include in the email
 */
export async function sendWelcomeCodeEmail(email, promoCode) {
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://girlsecret.co.uk';
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <div style="text-align: center;">
        <a href="${siteUrl}/shop?promo=${promoCode}" class="button">
          Start Shopping →
        </a>
      </div>
    ...
    <div class="footer">
      ...
      <p>
        <a href="${siteUrl}/unsubscribe" style="color: #666;">Unsubscribe</a> |
        <a href="${siteUrl}/privacy" style="color: #666;">Privacy Policy</a>
      </p>
    </div>
  </div>
</body>
  ...
  const text = `Welcome to GirlSecret!
...
export async function sendEmail(emailData) {
  ...
  try {
    ...
    switch (emailService) {
      ...
      default:
        throw new Error(`Unsupported email service: ${emailService}. Supported: sendgrid, ses, resend`);
    }
  } catch (error) {
...
// Mailgun integration removed due to security vulnerabilities
// Use Resend, SendGrid, or AWS SES instead
